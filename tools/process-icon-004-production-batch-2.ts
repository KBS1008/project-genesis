...
const productionRoot = path.join(projectRoot, 'docs/design/research/production/batch-2');
...
const BATCH_2_DETAILED_TECHNOLOGY_IDS = Object.freeze([
  'basic_woodworking',
  'industrial_assembly',
  'smart_grid',
  'warehouse_systems',
...
async function main(): Promise<void> {
  ...
  for (const technologyId of BATCH_2_DETAILED_TECHNOLOGY_IDS) {
  ...
  const alphaReportPath = path.join(productionRoot, 'ICON_004_PRODUCTION_BATCH_2_ALPHA_REPORT.json');
  await writeAlphaReportJson(alphaReports, alphaReportPath);

  const manifest = {
    batchVersion: '2026-09-19-icon-004-production-batch-2-v1',
    ...
    batch: 'batch-2',
    detailedTechnologyIds: BATCH_2_DETAILED_TECHNOLOGY_IDS,
    cumulativeDetailedCount: 14,
  ...
  await writeFile(
    path.join(productionRoot, 'ICON_004_PRODUCTION_BATCH_2_MANIFEST.json'),
  ...
  if (failed.length > 0) {
    ...
    console.log(`ICON-004 Batch 2: ${alphaReports.length} primaries alpha PASS`);
  }
}
