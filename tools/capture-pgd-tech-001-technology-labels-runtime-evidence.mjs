/* global document, console, process */
import path from 'node:path';
import { pathToFileURL, fileURLToPath } from 'node:url';

const ENABLED_TECHNOLOGY_IDS = [
  'advanced_metallurgy',
  'basic_woodworking',
  'circuit_design',
  'coal_efficiency',
  'corporate_management',
  'crop_optimization',
  'distribution_networks',
  'executive_leadership',
  'factory_automation',
  'financial_planning',
  'industrial_assembly',
  'intermodal_logistics',
  'organic_chemistry',
  'polymer_science',
  'precision_machining',
  'predictive_analytics',
  'process_automation',
  'renewable_energy',
  'semiconductor_process',
  'smart_grid',
  'sustainable_agriculture',
  'warehouse_systems',
];

async function loadChromium() {
  try {
    const playwright = await import('playwright');
    return playwright.chromium;
  } catch {
    const localPlaywright = path.join(
      path.dirname(fileURLToPath(import.meta.url)),
      'capture-evidence-tmp/node_modules/playwright/index.mjs',
    );
    const playwright = await import(pathToFileURL(localPlaywright).href);
    return playwright.chromium;
  }
}

const chromium = await loadChromium();
const projectRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const evidenceDir = path.join(projectRoot, 'docs/architecture/reviews/evidence');
const webOrigin = process.env.PG_WEB_ORIGIN ?? 'http://127.0.0.1:3000';
const savePath = path.join(projectRoot, 'saves/e2e-m11-phase6-production-closeout.json');

async function bootstrapSession(page) {
  const loadResponse = await page.request.post(`${webOrigin}/api/session/load`, {
    data: { filePath: savePath },
  });
  if (loadResponse.ok()) {
    return;
  }
  throw new Error(`Session load failed: ${loadResponse.status()} ${await loadResponse.text()}`);
}

async function assertNoRawTechnologyIds(page) {
  const blockerTexts = await page.evaluate(() => {
    const rows = [...document.querySelectorAll('.pg-operation-hint-row, .pg-operation-hint-copy')];
    return rows
      .map((row) => row.textContent ?? '')
      .filter((text) => text.includes('Forschung'));
  });

  const leaks = [];
  for (const text of blockerTexts) {
    for (const id of ENABLED_TECHNOLOGY_IDS) {
      if (text.includes(id)) {
        leaks.push({ id, text: text.slice(0, 140) });
      }
    }
  }

  if (leaks.length > 0) {
    throw new Error(`Raw technology IDs in blocker copy: ${JSON.stringify(leaks)}`);
  }

  return { blockerCount: blockerTexts.length, sample: blockerTexts.slice(0, 5) };
}

async function captureBuildings(viewport, fileName) {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport });
  await bootstrapSession(page);
  await page.goto(`${webOrigin}/game?screen=buildings`, { waitUntil: 'domcontentloaded' });
  await page.getByRole('heading', { name: 'Baukatalog' }).waitFor({ timeout: 60_000 });
  const assertion = await assertNoRawTechnologyIds(page);
  await page.screenshot({ path: path.join(evidenceDir, fileName), fullPage: true });
  await browser.close();
  return assertion;
}

async function captureResearch(viewport, fileName) {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport });
  await bootstrapSession(page);
  await page.goto(`${webOrigin}/game?screen=research`, { waitUntil: 'domcontentloaded' });
  await page.getByRole('heading', { name: 'Forschungskatalog' }).waitFor({ timeout: 60_000 });
  const assertion = await assertNoRawTechnologyIds(page);
  await page.screenshot({ path: path.join(evidenceDir, fileName), fullPage: true });
  await browser.close();
  return assertion;
}

const buildingsDesktop = await captureBuildings(
  { width: 1440, height: 900 },
  'PGD_TECH_001_TECHNOLOGY_LABELS_BUILDINGS_DESKTOP_1440x900.png',
);
const buildingsNarrow = await captureBuildings(
  { width: 480, height: 900 },
  'PGD_TECH_001_TECHNOLOGY_LABELS_BUILDINGS_NARROW_480x900.png',
);
const researchDesktop = await captureResearch(
  { width: 1440, height: 900 },
  'PGD_TECH_001_TECHNOLOGY_LABELS_RESEARCH_DESKTOP_1440x900.png',
);

console.log(
  JSON.stringify(
    {
      webOrigin,
      buildingsDesktop,
      buildingsNarrow,
      researchDesktop,
    },
    null,
    2,
  ),
);
