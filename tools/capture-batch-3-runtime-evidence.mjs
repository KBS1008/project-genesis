...
const webOrigin = process.env.PG_WEB_ORIGIN ?? 'http://localhost:3011';
...
async function loadSession(page) {
  ...
  const newResponse = await page.request.post(`${webOrigin}/api/session/new`, {
    data: { name: 'Batch 3 Runtime Evidence' },
  });
  if (!newResponse.ok()) {
    throw new Error(
...
async function capture(viewport, fileName, minIcon003 = 14) {
...
async function captureMixedInfra(viewport, fileName) {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport });
  await loadSession(page);
  await ensureBuildingsCatalog(page);
  const wartung = page.getByText('Wartungswerk', { exact: true }).first();
  const port = page.getByText('Hafenanlage', { exact: true }).first();
  await wartung.scrollIntoViewIfNeeded();
  await port.scrollIntoViewIfNeeded();
  await page.waitForFunction(() => {
    const img = document.querySelector('img[src*="ICON-003-maintenance_facility"]');
...
await capture({ width: 1280, height: 900 }, 'BUILDING_BATCH_3_CATALOG_DESKTOP.png', 14);
await captureMixedInfra({ width: 1280, height: 900 }, 'BUILDING_BATCH_3_CATALOG_MIXED_INFRA_FALLBACK.png');
await capture({ width: 480, height: 900 }, 'BUILDING_BATCH_3_CATALOG_NARROW.png', 12);

console.log('Captured batch-3 runtime evidence to', evidenceDir);
