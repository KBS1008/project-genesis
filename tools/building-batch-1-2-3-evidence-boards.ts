...
const ALL = [...BATCH_1, ...BATCH_2, ...BATCH_3];
...
function batchFor(buildingTypeId: string): string {
  if ((BATCH_1 as readonly string[]).includes(buildingTypeId)) {
    return 'batch-1';
  }
  if ((BATCH_2 as readonly string[]).includes(buildingTypeId)) {
    return 'batch-2';
  }
  return 'batch-3';
}
...
async function composePrimaryFamilyBoard(): Promise<void> {
  ...
  const target = path.join(evidenceDir, 'BUILDING_BATCH_1_2_3_PRIMARY_FAMILY_BOARD.png');
...
async function composeCompactFamilyBoard(): Promise<void> {
  ...
  const target = path.join(evidenceDir, 'BUILDING_BATCH_1_2_3_COMPACT_FAMILY_BOARD.png');
  await base.composite(composites).png().toFile(target);
  console.log('Wrote', target);
}
