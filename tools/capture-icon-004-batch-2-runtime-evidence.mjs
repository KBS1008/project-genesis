...
const EXPECTED_DETAILED_PRIMARIES = 14;
...
async function loadSession(page) {
  ...
  const newResponse = await page.request.post(`${webOrigin}/api/session/new`, {
    data: { name: 'ICON-004 Batch 2 Runtime Evidence' },
  });
  if (!newResponse.ok()) {
...
async function captureMixedCoverage(page) {
  await page.evaluate(() => {
    const names = ['Basic Woodworking', 'Precision Machining', 'Corporate Management'];
  ...
  await page.screenshot({
    path: path.join(evidenceDir, 'ICON_004_PRODUCTION_BATCH_2_MIXED_COVERAGE.png'),
    fullPage: false,
  });
}
...
await captureViewport({ width: 1440, height: 900 }, 'ICON_004_PRODUCTION_BATCH_2_RESEARCH_DESKTOP.png');
await captureViewport({ width: 480, height: 900 }, 'ICON_004_PRODUCTION_BATCH_2_RESEARCH_NARROW.png');
await captureMixedViewport();

console.log('ICON-004 Batch 2 runtime evidence captured.');
